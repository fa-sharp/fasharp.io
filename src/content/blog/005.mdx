...
image:
  url: "https://images.unsplash.com/photo-1512733596533-7b00ccf8ebaf?auto=format&fit=crop&w=600&q=60"
  alt: "a piano"
pubDate: 2023-09-05
...
Finally, for our keyboard users, we can style the outline that automatically appears when they're navigating with the 'tab' key. When a keyboard user focuses on a specific item, it triggers the `:focus-visible` state (note this is different from the `:focus` state, which will trigger for both mouse and keyboard users). Using this state, let's style the outline as a bright, contrasting color, and also add an `outline-offset` that will separate the outline from the button to make it super clear:
...
Questionable color and design choices aside, we've created something much more accessible and clear - not just for keyboard users, but for all our users!
