...
- The `title` attribute adds a visual tooltip to the button, which appears when the user hovers over the button with their cursor. So with the `title` attribute, we can generally assume that the user knows the _visual context_ of the button. (i.e. whether the button is part of a toolbar, or if it's next to a specific item, etc..)
- With the `aria-label` property, we'll want to be more descriptive and include some context if possible. Screen readers and other assistive devices will default to reading the `aria-label` property when there is one.
