...
Notice we didn't put the word "button" in our labels. Because we're correctly using the `<button>` HTML element, a screen reader will already let the user know that this is a button, so there's no need to duplicate that info in the label!
