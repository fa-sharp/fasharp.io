---
title: Let's talk about icon buttons
slug: icon-buttons-a11y
status: published
description: "Exploring icon buttons and accessibility concerns"
image:
...
"Icon buttons" are buttons that only appear as an icon with no visual, textual label. We often find them in places where space is limited - e.g. toolbars at the top of an app, or next to individual elements in a list of items. Think save buttons 💾, delete buttons 🗑️, and like buttons ❤️. The theme toggle button at the top of this website (the sun/moon icon) is a prime example.
...
This will clear out the default background, border, and padding of the `<button>` element, and add a helpful visual cue with the pointer cursor. You could always adjust the `padding` and `border` properties as needed, depending on how you want the icon button to look. And `display: inline-flex` makes it a bit easier to adjust the height and width of the button, and it also creates a [flexbox](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Flexbox) within the button that can help with positioning things.
...
<Icon
  aria-label="calendar plus icon"
  name="fa6-solid:calendar-plus"
  size={24}
/>
<Icon
  aria-label="calendar minus icon"
  name="fa6-solid:calendar-minus"
  size={24}
/>
...
Since an icon button won't have any clear, visual text label next to it, we need to make extra sure to add some _other_ labels in our HTML, including (but not limited to) the `title` and `aria-label` attributes:

- The `title` attribute adds a visual tooltip to the button, which appears when the user hovers over the button with their cursor. So with the `title` attribute, we can generally assume that the user knows the _visual context_ of the button. (i.e. is the button part of a toolbar, or is it next to a specific item? etc..)
- With the `aria-label` property, though, we can make no such assumption. Screen readers and other assistive devices will default to reading the `aria-label` property when there is one. So, in contrast to the `title` attribute, we'll want to make sure that the `aria-label` is as descriptive as possible.

```html
<button class="icon-button" title="Save" aria-label="Save the current task">
  <svg>...(your save icon SVG)...</svg>
</button>
```

Notice we didn't put the word "button" in our labels. Because we're correctly using the `<button>` HTML element, a screen reader or other assistive software will already let the user know that this is a button, so there's no need to duplicate that info in the label!
